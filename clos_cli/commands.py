...
def route_command(command: str, seed: int, ticks: int, stream: bool = False, manifest: str = "", genome: str = "default", scenario: str = "shock_world"):
    if command == "run": run_experiment(manifest)
    elif command == "run-matrix": run_matrix(manifest)
    elif command == "school": run_school()
    elif command == "demo": run_demo(seed, ticks, stream, genome, scenario)
