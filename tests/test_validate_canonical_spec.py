...
class TestCriticalFilesRegistryCoverage:
    ...
    def test_load_critical_files_returns_known_entries(self):
        """Celowo BEZ literalu licznosci w tescie ani w nazwie (B4C-2 (10),
        znalezisko: cztery testy przypinaly licznosc rejestru, jeden z nich
        wbrew wlasnemu docstringowi obiecujacemu odpornosc na zmiane).
        Licznosc jest zadeklarowana WYLACZNIE w hard_halt.py:347 - ten test
        sprawdza TRESC (obecnosc konkretnych, znanych pozycji), nie licznosc.
        Historia wzrostu (dla kontekstu, nie do przypinania w kodzie):
